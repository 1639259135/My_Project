...
<table width="97%" border="1" >
  ...
  <%--数据展示，单行：tabtd1；双行：tabtd2 --%>
	<s:if test="#session.page.courseTypeList != null">
		<s:iterator var="courseType" value="#session.page.courseTypeList" status="ct">
			<tr class="<s:property value="%{#ct.odd ? 'tabtd2' : 'tabtd1'}"/>">
				...
				<td width="11%" align="center">
					<s:url var="queryByCTID" value="queryByCTID.action">
						<s:param name="courseType.courseTypeId"><s:property value="#courseType.courseTypeId"/> </s:param>
					</s:url>
					<s:a href="%{queryByCTID}"><img src="${pageContext.request.contextPath}/images/button/modify.gif" class="img" /></s:a>
				</td>
			</tr>
...
<table border="0" cellspacing="0" cellpadding="0" align="center">
  <tr>
    <td align="right">
		...
		<span>
			<s:url var="typeFirstPage" value="typePage.action">
				<s:param name="page.pageCode">1</s:param>
			</s:url>
        	<s:a href="%{typeFirstPage}">[首页]</s:a>&nbsp;&nbsp;

			<s:if test="#session.page.pageCode != 1">
				<s:url var="typeLastPage" value="typePage.action">
					<s:param name="page.pageCode"><s:property value="#session.page.pageCode - 1"/> </s:param>
				</s:url>
				<s:a href="%{typeLastPage}">[上一页]</s:a>&nbsp;&nbsp;
			</s:if>
			<s:else>
				<s:a href="#">[上一页]</s:a>&nbsp;&nbsp;
			</s:else>
			...
			<s:if test="#session.page.pageCode != 1">
				<s:url var="typeNextPage" value="typePage.action">
					<s:param name="page.pageCode"><s:property value="#session.page.pageCode + 1"/> </s:param>
				</s:url>
				<s:a href="%{typeNextPage}">[下一页]</s:a>&nbsp;&nbsp;
			</s:if>
			<s:else>
				<s:a href="#">[下一页]</s:a>&nbsp;&nbsp;
			</s:else>

			<s:url var="typeEndPage" value="typePage.action">
				<s:param name="page.pageCode"><s:property value="#session.page.totalPages"/> </s:param>
			</s:url>
            <s:a href="%{typeEndPage}">[尾页]</s:a>
        </span>
    </td>
  </tr>
</table>
